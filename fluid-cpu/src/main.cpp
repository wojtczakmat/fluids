...
const static double VIEW_HEIGHT = 1.5*800.f;
...
void CalcPressureDiff(float& minP, float& maxP)
{
	maxP = -INF;
	minP = INF;
...
void LogPressures()
{
	...
	cout << "Min pressure " << minP << ", max pressure " << maxP << ", difference " << maxP - minP << endl;
}
...
void Render(void)
{
	...
	float minP, maxP, diff;
	CalcPressureDiff(minP, maxP);
	diff = maxP - minP;

	for(auto &p : particles)
	{
		float red = 2*(p.p - minP) / diff;
		glColor4f(red, 1.0f - red, 0.0f, 1);
...
{
    if (argc >= 2) {
        DAM_PARTICLES = strtol(argv[1], NULL, 10);
    }
